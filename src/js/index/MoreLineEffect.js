...
export function setMoreLineEffect(enable) {
    ...
    var a, c, u, m = document.createElement("canvas"), d = t(), docId = "moreLineEffect_core"/* + d.l*/, r = m.getContext("2d"),
        x = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || function (n) {
            window.setTimeout(n, 1e3 / 45)
        }, w = Math.random, y = {x: null, y: null, max: 75000};
    m.id = docId, m.style.cssText = "position:fixed;top:0;left:0;z-index:" + d.z + ";opacity:" + d.o, getElement("moreLineEffect").appendChild(m), sizeChange(), window.onmousemove = function (n) {
        n = n || window.event, y.x = n.clientX, y.y = n.clientY
    }, window.onmouseout = function () {
        y.x = null, y.y = null
    };
    sizeChangeFunc=sizeChange;
    $(window).on('resize',sizeChangeFunc);
    ...
    u = s.concat([y]), setTimeout(function () {
        ...
        isRun=false;
        if (sizeChangeFunc!=null)
            $(window).off('resize',sizeChangeFunc);
        document.getElementById("moreLineEffect_core").remove();
    }
}
